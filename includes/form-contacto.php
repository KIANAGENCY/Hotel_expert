...
$pref = $pref ?? 'asesoria';
$lock_interest = $lock_interest ?? false;
$submit_label = $submit_label ?? 'Enviar solicitud';
?>
<form action="<?= e(url('procesar-contacto.php')) ?>" method="post" class="grid gap-4">
    ...
    <?php if ($lock_interest && $pref === 'muestra'): ?>
        <input type="hidden" name="interes" value="muestra">
        <div class="sample-interest-summary">
            <span>Tipo de solicitud</span>
            <strong>Muestra del Sistema ELAH</strong>
        </div>
    <?php else: ?>
        <label class="block">
    ...
    <div class="flex flex-wrap gap-3">
        <button class="btn-primary justify-center py-3.5 text-base" type="submit"><?= e($submit_label) ?></button>
